...
function CoursesPageContent() {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        // The catalog is paginated server-side (DRF PageNumberPagination), but search/price
        // filtering here runs client-side against the full `courses` array — so this follows
        // `next` until every page is collected rather than silently truncating to one page.
        // Fine while the catalog is small; revisit with server-side search once it grows.
        let allCourses = [];
        let url = "courses/";
        while (url) {
        ...
        <CardHeader className="pb-2">
          <div className="flex justify-between items-start">
            ...
            {course.price == 0 ? (
              ...
              <Badge
                variant="outline"
                className="bg-blue-100 text-blue-800 border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-900"
              >
                ${course.price}
              </Badge>
            )}
          ...
          <CardDescription
            className="line-clamp-2 text-muted-foreground mt-1"
