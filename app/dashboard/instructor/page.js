...
//         ...formData,
//         course: selectedCourse.id,
//         duration: parseInt(formData.contents.reduce((acc, c) => acc + (c.duration || 0), 0)) || 0,
//       };
//       console.log(payload);
...
//         setSelectedCourse((prev) => ({ ...prev, modules: updatedModules }));
//         setIsAddLessonOpen(false);
//         setSuccess("Lesson and contents added successfully!");
//       }
//     } catch (err) {
//       console.error("Add Lesson Error:", err.response?.data);
//       setError(err.response?.data?.detail || "Failed to add lesson and contents");
//     }
//   };
...
//       };
//       console.log(payload);
//       const res = await axiosInstance.put(`/lessons/${currentLesson.id}/`, payload);
...
//         setSelectedCourse((prev) => ({ ...prev, modules: updatedModules }));
//         setIsEditOpen(false);
//         setSuccess("Lesson and contents updated successfully!");
//       }
//     } catch (err) {
//       console.error("Update Lesson Error:", err.response?.data);
//       setError(err.response?.data?.detail || "Failed to update lesson and contents");
//     }
//   };

//   const handleDeleteLesson = async (lessonId) => {
//     if (!confirm("Delete this lesson and its contents?")) return;
//     try {
//       await axiosInstance.delete(`/lessons/${lessonId}/`);
//       const updatedModules = selectedCourse.modules.map((mod) => ({
//         ...mod,
...
export default function InstructorDashboard() {
  ...
  const handleAddLesson = async (formData) => {
    try {
      ...
      console.log("Add Lesson Payload:", payload);
      const res = await axiosInstance.post("/lessons/", payload);
      ...
      if (res.status === 201) {
        ...
        setSelectedCourse((prev) => ({ ...prev, modules: updatedModules }));
        setIsAddLessonOpen(false);
        setSuccess("Lesson, contents, and resources added successfully!");
      }
    } catch (err) {
      console.error("Add Lesson Error:", err.response?.data);
      setError(err.response?.data?.detail || "Failed to add lesson, contents, or resources");
    }
  };
  ...
  const handleUpdateLesson = async (formData) => {
    try {
      ...
      if (res.status === 200) {
        ...
        setSelectedCourse((prev) => ({ ...prev, modules: updatedModules }));
        setIsEditOpen(false);
        setSuccess("Lesson, contents, and resources updated successfully!");
      }
    } catch (err) {
      console.error("Update Lesson Error:", err.response?.data);
      setError(err.response?.data?.detail || "Failed to update lesson, contents, or resources");
    }
  };

  const handleDeleteLesson = async (lessonId) => {
    if (!confirm("Delete this lesson, its contents, and resources?")) return;
    try {
      await axiosInstance.delete(`/lessons/${lessonId}/`);
      const updatedModules = selectedCourse.modules.map((mod) => ({
        ...mod,
