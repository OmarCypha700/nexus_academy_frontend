...
import { FileQuestion, FileUp, Plus } from "lucide-react";
import QuizPreview from "./QuizPreview";
import QuestionList from "@/app/components/dashboard/quiz/QuestionList";
...
export default function QuizList({
  ...
  const [isPreviewOpen, setIsPreviewOpen] = useState(false);
  const [isQuestionListOpen, setIsQuestionListOpen] = useState(false);
  const [aikenQuiz, setAikenQuiz] = useState(null);
  const [selectedQuiz, setSelectedQuiz] = useState(null);
  ...
  const handleDeleteQuiz = async (quizId) => {
    if (!confirm("Delete this quiz?")) return;
    try {
      // H8: was `queryId`, an undefined variable — the parameter is `quizId`. This threw
      // a ReferenceError on every call, silently breaking quiz deletion from this view.
  ...
  return (
    <div className="space-y-4">
      ...
      <AikenImportExportDialog
        ...
        mode="quiz"
        open={!!aikenQuiz}
        onOpenChange={(next) => !next && setAikenQuiz(null)}
        onImported={fetchQuizzes}
      />
    </div>
  );
}