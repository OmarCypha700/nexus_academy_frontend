...
export default function InstructorSignupForm() {
  const [formData, setFormData] = useState({
    ...
    email: "",
    password: "",
    // H7: previously this was only set inside handleChange, meaning it was applied on the
    // NEXT field edit after mount rather than being present from the start — a user who
    // filled every field via autofill/paste-then-submit without triggering a subsequent
    // change event would silently register as a student instead of an instructor.
    role: "instructor",
  });
  const [error, setError] = useState(null);
